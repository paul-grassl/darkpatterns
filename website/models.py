...
class ControlAndDeliberationData(db.Model):
    ...
    participantId = db.Column(db.Integer, db.ForeignKey('demographic_data.id'), nullable=False)
    currentWebsite = db.Column(db.String(50), nullable=False)
    manipulationCheck1 = db.Column(db.String(1), nullable=False)
    manipulationCheck2 = db.Column(db.String(3), nullable=False)
    ...
    def __repr__(self):
        return f"controlAndDeliberationData('{self.participantId}', '{self.id}', '{self.currentWebsite}', '{self.manipulationCheck1}', '{self.manipulationCheck2}', '{self.deliberation}', '{self.perceivedControlQ1}', '{self.perceivedControlQ2}', '{self.perceivedControlQ3}', '{self.perceivedControlQ4}', '{self.perceivedControlQ5}')"
...
class PrivacyConcernsData(db.Model):
    ...
    browserPlugin = db.Column(db.String(1), nullable=False)
    whichDevice = db.Column(db.String(10), nullable=False)

    def __repr__(self):
        return f"privacyConcernsData('{self.participantId}', '{self.id}', '{self.privacyConcernsQ1}', '{self.privacyConcernsQ2}', '{self.privacyConcernsQ3}', '{self.correctDisplayed}', '{self.browserPlugin}', '{self.whichDevice}')"
